 ...
 * SwipeBox - Class
 *
 * Класс для отображения и загрузки с сервера фото
 *
 * JavaScript
 *
 *
...
/**
* Это функция-конструктор SwipeBox().
*
* Данный конструктор инициализирует обьект
*/
function SwipeBox(params)
{
    if(!params){
        return;
    }
    ...
    // a GET AJAX request
    var url = BSA.lb.getMsg('urlBase') + params.url;
    $.get(url, {
        path_large: params.path_large,
        path_thumbs: params.path_thumbs
    }, function(response){

        // response.data holds the photos
        $.each(response.data, function(){
            var thumb = BSA.lb.getMsg('urlBase') + '/' + this.thumb;
            var large = BSA.lb.getMsg('urlBase') + '/' + this.large;
            data.push({
                thumb: thumb,
                large: large
            });
    ...
    gallery.on('data-ready window-resized page-turned', function(event, direction){
        ...
        $.each(set, function(){
            ...
            // build the cache
            cache.push('<a href="' + this.large + '" class="swipebox divbox-image"' +
            ...
            if(event.type == 'window-resized'){
                ...
                gallery.fadeOut(function(){
                    ...
                    if($('#gallery .swipebox').swipebox){
                        ...
                        var divBoxs = BSA.ScriptInstances['DivBox'];
                        if(divBoxs){
                            var divBox = divBoxs[0];
                            divBox.iniDivBox();
                        }
...
// пр. {SwipeBox: [new SwipeBox(), ... ,new SwipeBox()]}
SwipeBox.RegRunOnLoad = function() {
    
    // Получим параметры для создания обьекта
    var params = BSA.ScriptParams['SwipeBox'];
    // Ф-ия создания обьектов по их параметрам
    var createObject = function(param){
        var swipeBox = BSA.ScriptInstances['SwipeBox'];
        if(swipeBox){
            swipeBox.push(new SwipeBox(param));
        }else{
            BSA.ScriptInstances['SwipeBox'] = [new SwipeBox(param)];
        }
    }
    // Создание обьектов
    if(params){
...
runOnLoad(SwipeBox.RegRunOnLoad);
